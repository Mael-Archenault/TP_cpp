...
FightState::FightState(Game* context,Trainer* player)
    ...
    Pokedex* pokedex = context->getPokedex();
    int maxIndex = pokedex->at(pokedex->size()-1).getId();
    int randomIndex = rand() % maxIndex;
    std::cout<<randomIndex<<std::endl;
    Pokemon test = context->getPokedex()->getCloneById(randomIndex);
    opponent = new Pokemon(test);
    context = context;
